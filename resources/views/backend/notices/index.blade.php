                        ...
                        <div class="btn-group" role="group" aria-label="Toolbar button groups">
                            <div class="btn-group" role="group">
                                ...
                                <div class="dropdown-menu" aria-labelledby="btnGroupToolbar">
                                    <a class="dropdown-item" href="{{ route('backend.notices.trashed') }}">
                        ...
                        <tbody>
                            @forelse($notices as $key=>$notice)
                                <tr>
                                    <td>{{ $key + 1 }} </td>
                                    <td>{{ $notice->title }} </td>
                                    {{-- <td> {!! html_entity_decode($notice->body) !!}</td> --}}
                                    <td>{{ $notice->created_at }} </td>
                                    <td class="text-right">
                                        <a href='{{ route('backend.notices.edit', $notice->id) }}'
                                            class='btn btn-sm btn-primary mt-1' data-toggle="tooltip"
                                            title="Edit {{ ucwords(Str::singular($module_name)) }}"><i
                                                class="fas fa-wrench"></i></a>
                                        <a href='{{ route('backend.notices.show', $notice->id) }}'
            ...
            <div class="row">
                <div class="col-7">
                    <div class="float-left">
                        Total {{ $notices->total() }} {{ ucwords($module_name) }}
                    </div>
                </div>
                <div class="col-5">
                    <div class="float-right">
